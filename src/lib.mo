...
/// the safe side (floor for what you pay out, ceil for what you collect).
/// `multiplyNatByFloatMin`/`multiplyNatByFloatMax` round the `Float` product
/// directly, which is a safe bound as long as `value` and the product both
/// stay below `2 ** 53`; `multiplyNatByFloatMinSafe`/`multiplyNatByFloatMaxSafe`
/// add a fallback to exact integer arithmetic above that limit, where no
/// rounding can creep in at all, at the cost of being slower whenever that
/// fallback is taken.
///
/// ```motoko name=import
...
module {
  ...
  /// mathematical product of `value` and `multiplier`, *provided* that product
  /// stays below `2 ** 53`.
  ///
  /// Below that limit every integer is representable as a `Float`, so
  /// converting `value` is exact and the product is off from the true value by
  /// less than one unit — flooring it absorbs the difference. This is also what
  ...
  ///
  /// This function does *not* switch to exact integer arithmetic once `value`
  /// or the product crosses `2 ** 53`: it always computes the product as a
  /// `Float`, so for large operands the result can drift from the true product
  /// by more than one unit, in either direction. Use `multiplyNatByFloatMinSafe`
  /// when `value` or the product may exceed `2 ** 53` and the floor still has to
  /// be exact.
  ///
  /// Example:
  ...
  /// `value` and `multiplier`, *provided* that product stays below `2 ** 53`.
  ///
  /// Below that limit the guarantee is the weaker one that the result is the
  /// ceiling of the *rounded* product, which can be one unit below the exact
  /// product. That is the deliberate trade for treating a `Float` as the
  ...
  /// or the product crosses `2 ** 53`: it always computes the product as a
  /// `Float`, so for large operands the result can drift from the true product
  /// by more than one unit, in either direction — including *below* it, which
  /// is precisely what a ceiling must never do. Use `multiplyNatByFloatMaxSafe`
  /// when `value` or the product may exceed `2 ** 53` and the ceiling still has
  /// to be a genuine upper bound.
  ///
  /// Example:
  /// ```motoko include=import
  ...
  ///
  /// This is the counterpart to `multiplyNatByFloatMin` that stays correct
  /// once `value` or the product can exceed `2 ** 53`; reach for the plain
  /// version instead when operands are known to stay under that limit, since
  /// it skips the exact-arithmetic fallback entirely.
  ///
  /// Example:
  /// ```motoko include=import
  /// let payout = FinancialMath.multiplyNatByFloatMinSafe(4_000, 0.0125); // 50
  /// ```
  ///
  /// `multiplier` is expected to be a non-negative price. A negative
  /// `multiplier` returns the absolute value of the floored product rather than
  ...
  ///
  /// Like `multiplyNatByFloatMinSafe`, products below `2 ** 53` are computed in
  ...
  /// must never be short.
  ///
  /// This is the counterpart to `multiplyNatByFloatMax` that stays correct
  /// once `value` or the product can exceed `2 ** 53`; reach for the plain
  /// version instead when operands are known to stay under that limit, since
  /// it skips the exact-arithmetic fallback entirely.
  ///
  /// Example:
  /// ```motoko include=import
  /// let required = FinancialMath.multiplyNatByFloatMaxSafe(4_000, 0.0125); // 50
  /// ```
  ///
  /// `multiplier` is expected to be a non-negative price. A negative
  /// `multiplier` returns the absolute value of the ceiled product rather than
